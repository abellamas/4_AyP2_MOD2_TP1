...
def index(request):
    ...
    if request.method == "POST":
        ...
        if material_form.is_valid() & dimensions_form.is_valid():
            ...
            print(panel.data())
            print(f'Rigidez: {panel.stiffness}')
            print(f'Masa superficial: {panel.mass_sup}')
            print(f'Frecuencia crítica: {panel.freq_critic}')
            print(f'Frecuencia de resonancia: {panel.freq_res}')
            print(panel.cramer_model(f_per_thirds))
            # print(material.density)
            # print(lx, ly, thickness)
